...
if __name__ == "__main__":
    ...
    if disable_nvshmem:
        ...
        # Check if it's the system installation path
        if nvshmem_dir == "/usr":
            # Use system NVSHMEM paths
            nvshmem_include = "/usr/include/nvshmem_12"
            nvshmem_lib = "/usr/lib/x86_64-linux-gnu/nvshmem/12"
        else:
            # Use custom NVSHMEM paths
            nvshmem_include = f"{nvshmem_dir}/include"
            nvshmem_lib = f"{nvshmem_dir}/lib"
    ...
    if disable_nvshmem:
        ...
        include_dirs.extend([nvshmem_include])
        library_dirs.extend([nvshmem_lib])
        nvcc_dlink.extend(
            [
                "-dlink",
                f"-L{nvshmem_lib}",
                # "-lnvshmem_host",
                f"{nvshmem_lib}/libnvshmem_device.a",
            ]
        )
        extra_link_args.extend(
            [
                "-lnvshmem_host",
                # f"{nvshmem_lib}/nvshmem_bootstrap_uid.so",
                f"{nvshmem_lib}/libnvshmem_device.a",
    ...
    if int(os.getenv("DISABLE_SM90_FEATURES", 0)):
        ...
        # CUDA 12 flags - device linking 필수
        nvcc_flags.extend(["-rdc=true", "--ptxas-options=--register-usage-level=10"])

        # Device linking을 위한 nvcc_dlink 설정 (PyTorch가 자동으로 dlink.o 생성)
        nvcc_dlink.extend(["-dlink"])

        # H100 링킹 문제 해결을 위한 추가 플래그
        extra_link_args.extend(
            [
                "-lcudart",
                "-lcuda",
                "-lcudadevrt",
                f"-L{os.environ.get('CUDA_HOME', '/usr/local/cuda-12.4')}/lib64",
                "-Wl,--no-as-needed",
            ]
        )
