...
def test_512_tokens(rank, world_size):
    """512 토큰만 테스트하는 간단한 함수"""
    ...
    dist.init_process_group(
        backend="nccl",
        init_method=f"tcp://localhost:29520",
        world_size=world_size,
        rank=rank,
    )

    print(f"🚀 [Rank {rank}] 512 토큰 테스트 시작")
    ...
    try:
        ...
        print(f"🔄 [Rank {rank}] 512 토큰 Dispatch 시작...")
        ...
        if len(dispatch_result) == 6:
            ...
            print(f"🔄 [Rank {rank}] 512 토큰 Combine 시작...")

            combine_result = wrapper.combine(
                dispatched_tensor, dispatch_handle, recv_topk_weights
            )

            print(f"🎉 [Rank {rank}] 512 토큰 테스트 완전 성공!")
            return True

    except Exception as e:
        print(f"❌ [Rank {rank}] 512 토큰 테스트 실패: {e}")
        return False
    finally:
...
def main():
    ...
    print("🚀 512 토큰 단일 테스트 시작")

    for rank in range(world_size):
        p = mp.Process(target=test_512_tokens, args=(rank, world_size))
        p.start()
        processes.append(p)
